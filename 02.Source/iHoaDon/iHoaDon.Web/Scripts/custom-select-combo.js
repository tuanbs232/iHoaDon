...
;(function ( $, window, document, undefined ) {
  ...
  var customSelectCombo = "customSelectCombo",
    defaults = {
      ...
      setInputValueOnTrigger: false, // set the value of the input when triggered
      triggerValues: ['Khác'], // option values on which to trigger the custom input
  ...
  CustomSelectCombo.prototype = {
    ...
    createCustomInput: function($el) {
      ...
      var input ; 
      if (self.options.customInputTarget){
    	  input = jQuery(document.getElementById(self.options.customInputTarget));
      }
      if (!input){
      	input = $("<input type='text' class='custom-select-combo-input' style='display:none;'>")
      }
   //   var placeholder = $el.find('option').filter(function() {
    //    return !this.value || $.trim(this.value).length == 0;
    //  }).text()
   //   input.attr('placeholder', placeholder);
    //  input.val(self.options.inputValue); // set initial value to selection
   ...
   //   $el.parent().append(input)
    ...
    attachSelectEvents: function($el) {
      ...
      $el.change(function() {
        ...
        var selectVal = currentOption.text();
    //    input.val(optionVal);
